...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, tgt, output):
        """
        ...
        """
        shard_state = self._make_shard_state(tgt, output)
        _, batch_stats = self._compute_loss(**shard_state)

        return batch_stats

    def sharded_compute_loss(self, tgt, output, shard_size, normalization):
...
class NMTLossCompute(LossComputeBase):
    ...
    def _make_shard_state(self, tgt, output):
        return {
            "output": output,
            "target": tgt[:,1:],
        }
