...
def create_msqg_examples(root, split, debug=False):
...
def create_msqg_dataset(root, split, tokenizer, max_src_len, max_tgt_len, model_type, raw_cache_dir='', client_id=None,
                        pretrain=False, debug=False, **kwargs):
    if pretrain:
        return create_msqg_pretrain_dataset(root, split, tokenizer, max_src_len, model_type, raw_cache_dir, client_id, debug)

    if client_id is None:
        cache_dir = osp.join(raw_cache_dir, 'finetune', task, '_'.join([str(max_src_len), str(max_tgt_len), model_type.split('/')[-1]]), split)
    else:
        cache_dir = osp.join(raw_cache_dir, 'finetune', str(client_id), split)

    src_examples, tgt_examples = create_msqg_examples(root, split, debug)
    if osp.exists(cache_dir):
        ...
        src_encoded = tokenizer(src_examples, padding='max_length', truncation=True, max_length=max_src_len, return_tensors='pt')
        tgt_examples = split_sent(tgt_examples, eoq=tokenizer.eoq_token, tokenize=False)
        tgt_encoded = tokenizer(tgt_examples, padding='max_length', truncation=True, max_length=max_tgt_len, return_tensors='pt')
        num_non_padding = (tgt_encoded.input_ids != tokenizer.pad_token_id).sum(dim=-1)
    ...
    dataset = DictDataset({'token_ids': token_ids,
...
def create_msqg_pretrain_dataset(root, split, tokenizer, max_src_len, model_type, raw_cache_dir='', client_id=None, debug=False):
    if client_id is None:
        cache_dir = osp.join(raw_cache_dir, 'pretrain', task, '_'.join([str(max_src_len), model_type.split('/')[-1]]), split)
    else:
    ...
    if osp.exists(cache_dir):
        ...
        src_examples = split_sent(src_examples, eoq=tokenizer.eoq_token, tokenize=False)
        src_encoded = tokenizer(src_examples, padding='max_length', truncation=True, max_length=max_src_len, return_tensors='pt')
        num_non_padding = (src_encoded.input_ids != tokenizer.pad_token_id).sum(dim=-1)
        for i, pad_idx in enumerate(num_non_padding):
