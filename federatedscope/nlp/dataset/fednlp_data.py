...
def load_fednlp_data(config, client_config):
    ...
    data_collator = None
    if pretrain:
        if config.data.collator == 'mlm':
            data_collator = DataCollatorForMLM(tokenizer=tokenizer)
        elif config.data.collator == 'denoise':
    ...
    for client_id in tqdm(range(1, config.federate.client_num + 1)):
        ...
        max_tgt_len = config.data.max_pretrain_tgt_len if pretrain else getattr(config.data.max_tgt_len, cur_task, None)
