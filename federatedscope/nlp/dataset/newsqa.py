...
def encode(tokenizer, text_a, text_b, max_seq_len, max_query_len, added_trunc_size):
    ...
    return NewsQAEncodedInput(token_ids, token_type_ids, attention_mask, overflow_token_ids)


def create_newsqa_examples(root, split, debug=False):
    ...
    for para in data:
        ...
        if not is_impossible:
            ...
            if split == 'train':
                ...
                val_answer = [{'text': context[spans[i][0]: spans[i][1] + 1], 'answer_start': spans[i][0]}
                              for i in range(len(spans))]

        start_pos, end_pos, context_tokens = get_char_to_word_positions(
            context, train_answer, start_char_pos, is_impossible)
        examples.append(NewsQAExample(qa_id, question, context, train_answer, val_answer,
                                      start_pos, end_pos, context_tokens, is_impossible))
    return examples


def create_newsqa_dataset(root, split, tokenizer, max_seq_len, max_query_len, trunc_stride, model_type, cache_dir='',
                          client_id=None, pretrain=False, debug=False, **kwargs):
    if pretrain:
        return create_newsqa_pretrain_dataset(root, split, tokenizer, max_seq_len, model_type, cache_dir, client_id, debug)

    if client_id is None:
        save_dir = osp.join(cache_dir, 'finetune', task)
        cache_file = osp.join(save_dir, '_'.join([task, split, str(max_seq_len), str(max_query_len),
                                                  str(trunc_stride), model_type.split('/')[-1]]) + '.pt')
    else:
    ...
    if osp.exists(cache_file):
        ...
        examples = create_newsqa_examples(root, split, debug)
        ...
        for example_idx, example in enumerate(examples):
            ...
            for span_idx in range(len(spans)):
                for context_idx in range(spans[span_idx].context_len):
                    is_max_context_token = check_max_context_token(spans, span_idx,
                                                                   span_idx * trunc_stride + context_idx)
...
def create_newsqa_pretrain_dataset(root, split, tokenizer, max_seq_len, model_type, cache_dir='', client_id=None, debug=False):
    if client_id is None:
    ...
    if osp.exists(cache_file):
        ...
        examples = create_newsqa_examples(root, split, debug)
        texts = split_sent([e.context for e in examples], eoq=tokenizer.eoq_token)
