...
class DataCollatorForMLM(object):
    ...
    def __call__(self, examples):
        ...
        examples = {k: torch.stack([x[k] for x in examples]) for k in examples[0].keys()}
        token_ids = examples['token_ids']
        attention_mask = examples['attention_mask']
        labels = token_ids.clone()

        # We sample a few tokens in each sequence for masked-LM training
        # (with probability self.mlm_probability defaults to 0.15 in Bert/RoBERTa)
...
class DataCollatorForDenoisingTasks(object):
    ...
    """
    def __init__(self, tokenizer, mask_ratio=0.3, poisson_lambda=3.0, permutate_sentence_ratio=1.0):
        self.tokenizer = tokenizer
    ...
    def __call__(self, examples):
        ...
        if self.mask_ratio:
            ...
            num_non_padding = np.sum(token_ids != self.tokenizer.pad_token_id, axis=-1)
    ...
    def permutate_sentences(self, inputs):
        ...
        for i in range(inputs.shape[0]):
            full_stops = (inputs[i] == self.tokenizer.eoq_token_id) | (inputs[i] == self.tokenizer.eos_token_id)
            ...
            substitutions = np.random.permutation(num_sentences[0])[:num_to_permute[0]]
            ordering = np.arange(0, num_sentences[0])
            ordering[substitutions] = substitutions[np.random.permutation(num_to_permute[0])]

            index = 0
            for j in ordering:
                sentence = inputs[i, (sentence_ends[0][j - 1] if j > 0 else 0) : sentence_ends[0][j]]
                results[i, index : index + sentence.shape[0]] = sentence
                index += sentence.shape[0]

        num_non_padding = np.sum(results != self.tokenizer.pad_token_id, axis=-1)
...
class DataCollatorForPFedNLP(object):
    def __init__(self, tokenizer, mlm_probability=0.15, mask_ratio=0.3, poisson_lambda=3.0, permutate_sentence_ratio=1.0):
        self.mlm_collator = DataCollatorForMLM(tokenizer, mlm_probability)
        self.denoise_collator = DataCollatorForDenoisingTasks(tokenizer, mask_ratio, poisson_lambda, permutate_sentence_ratio)
