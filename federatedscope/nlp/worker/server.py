...
class FedNLPServer(Server):
    ...
    def save_best_results(self):
        if self._cfg.federate.save_to != '':
            self.aggregator.save_model(os.path.join(self._cfg.federate.save_to, 'global_model.pt'), self.state)
...
class PFedNLPServer(FedNLPServer):
    def __init__(self,
        ...
        self.tasks = [config.model.pretrain_tasks[0] if config.model.pretrain_tasks else None for _ in range(self.client_num)]
    ...
    def check_and_move_on(self, check_eval_result=False):
        ...
        if self.check_buffer(self.state, minimal_number, check_eval_result):
            if not check_eval_result:  # in the training process
                ...
                self.state += 1
                self.aggregator.update_round(self.state)  # for PerCFL
                if self.state % self._cfg.eval.freq == 0 and self.state != self.total_round_num:
                    #  Evaluate
                    logger.info(
                        'Server #{:d}: Starting evaluation at the end of round {:d}.'
...
class PFedNLPContrastServer(FedNLPServer):
    ...
    def _load_synth_data(self):
        ...
        logger.info('Loading synthetic data from \'{}\' (synthetic ratio: {})'.format(synth_dir, synth_ratio))
        with open(os.path.join(synth_dir, 'shapes.json')) as f:
            shapes = json.load(f)
        synth_feat_path = os.path.join(synth_dir, 'feature_{}.memmap'.format(synth_ratio))
        synth_tok_path = os.path.join(synth_dir, 'token_{}.memmap'.format(synth_ratio))
        ...
        synth_feats = {k: v for k, v in enumerate(torch.from_numpy(synth_feats)[:num_contrast])}
        synth_toks = {k: v for k, v in enumerate(torch.from_numpy(synth_toks)[:num_contrast])}
    ...
    def check_and_move_on(self, check_eval_result=False):
        ...
        if self.check_buffer(self.state, minimal_number, check_eval_result):
            if not check_eval_result:  # in the training process
                ...
                if self._cfg.model.task != 'pretrain' and self.contrast_monitor.stat == 2:
